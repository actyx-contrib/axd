...
const rightOrThrow = <A>(e: Either<io.Errors, A>): A => {
  if (isLeft(e)) {
...
const exec = async (args: string[]) => {
  try {
    return JSON.parse((await execa(`ax`, [`-j`].concat(args))).stdout);
  } catch (error) {
...
export const SettingsInput = {
  FromFile: (filePath: string): SettingsInputFile => ({
    key: "SettingsInputFile",
    path: filePath,
  }),
  FromValue: (value: unknown): SettingsInputValue => ({
    key: "SettingsInputValue",
    value: value,
  }),
...
export const Exec = {
  Nodes: {
    Ls: async (addr: string): Promise<Response_Nodes_Ls> => {
      const response = await exec([`nodes`, `ls`, `--local`, addr]);
  ...
  Settings: {
    Get: async (
      addr: string,
      scope: string
    ): Promise<Response_Settings_Get> => {
      const response = await exec([`settings`, `get`, scope, `--local`, addr]);
      return rightOrThrow(Response_Settings_Get.decode(response));
    },
    Set: async (
      addr: string,
      scope: string,
      settingsInput: SettingsInput
    ): Promise<Response_Settings_Set> => {
    ...
    Unset: async (
      addr: string,
      scope: string
    ): Promise<Response_Settings_Unset> => {
      const response = await exec([
        `settings`,
        `unset`,
        scope,
  ...
  Apps: {
    Package: async (path: string): Promise<Response_Apps_Package> => {
      const response = await exec([`apps`, `package`, path]);
    ...
    Deploy: async (
      ...
      packagePath: string,
      force?: boolean
    ): Promise<Response_Apps_Deploy> => {
      const response = await exec(
        [`apps`, `deploy`, packagePath, `--local`, addr].concat(
          force ? ["--force"] : []
        )
      );
    ...
    Undeploy: async (
      addr: string,
      appId: string
    ): Promise<Response_Apps_Undeploy> => {
      const response = await exec([`apps`, `undeploy`, appId, `--local`, addr]);
      return rightOrThrow(Response_Apps_Undeploy.decode(response));
    },
    Start: async (
      addr: string,
      appId: string
    ): Promise<Response_Apps_Start> => {
      const response = await exec([`apps`, `start`, appId, `--local`, addr]);
      return rightOrThrow(Response_Apps_Start.decode(response));
    },
    Stop: async (addr: string, appId: string): Promise<Response_Apps_Stop> => {
      const response = await exec([`apps`, `stop`, appId, `--local`, addr]);
      return rightOrThrow(Response_Apps_Stop.decode(response));
    },
    Ls: async (addr: string): Promise<Response_Apps_Ls> => {
      const response = await exec([`apps`, `ls`, `--local`, addr]);
      ...
      try {
        ...
        process.stdout.pipe(entryDecoder).on("data", (str) => {
          ...
          if (isLeft(entry)) {
            ...
            error = `error decoding log entry response: ${PathReporter.report(
              entry
            ).join(", ")}`;
            process.kill();
